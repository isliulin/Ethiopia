...
ROMDATA g_PicRom_CarCodeSet[] =
{
    ...
    {QSTR("Vehicle Code Setting"),   D_FONT(13),           QRect(  0,  0,640, 60),           Qt::black,                   Qt::yellow,                 CONTROL_LABEL,        ID_IGNORE                 },
    {QSTR("OK"),          D_COMMON_TITLE_FONT,  QRect(160,  380, 120, 50),        Qt::black,                   Qt::yellow,                 CONTROL_BUTTON,       ID_PICCCS_BUTTON_OK       },
    {QSTR("Quit"),          D_COMMON_TITLE_FONT,  QRect(360,  380, 120, 50),        Qt::black,                   Qt::yellow,                 CONTROL_BUTTON,       IDBTN_PICCCS_QUIT       },
    ...
    {QSTR("Clear"),          D_COMMON_TITLE_FONT,  QRect(500,  250, 100, 50),        Qt::black,                   Qt::yellow,                 CONTROL_BUTTON,       ID_PICCCS_BUTTON_CLEAR    },
...
void CCarCodeSetPage::OnOKUp()
{
    ...
    if(strData.length() == CARCODELENGTH)
    {
        ...
        if( strData.toInt() == 0 )
        {
            ...
            warningDlg.SetWarningStr(QSTR("Vehicle code ranges from 001 to 999！"));
    ...
    {
        CWarningDialog warningDlg;
        warningDlg.SetWarningStr(QSTR("Vehicle code must be") + QString::number(CARCODELENGTH) + QSTR("bits！"));
        warningDlg.setGeometry(m_pDlg->x()+int((m_pDlg->width()-400)/2),m_pDlg->y()+int((m_pDlg->height()-120)/2),400,120);
        warningDlg.exec();
        OnInitPage();
    }
...
void CCarCodeSetPage::OnUpdatePage()
{
    ...
    ((CLabel *)GetDlgItem(ID_PICCCS_LABEL_CURRENTCARCODE))->SetCtrlText(QSTR("Current Vehicle code:") + str);
}
...
void CCarCodeSetPage::OnNum1Up()
{
    ...
    {
        CWarningDialog warningDlg;
        warningDlg.SetWarningStr(QSTR("Vehicle code is out of range"));
        warningDlg.setGeometry(m_pDlg->x()+int((m_pDlg->width()-400)/2),m_pDlg->y()+int((m_pDlg->height()-120)/2),400,120);
        warningDlg.exec();
    }
}
