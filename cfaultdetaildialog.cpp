    ...
    ui(new Ui::CFaultDetailDialog)
{
    ...
    m_strCode.sprintf("%04d", g_faultsrom[romIdx].code);
    m_strGrade.sprintf("%d", g_faultsrom[romIdx].grade);
    m_strName = g_faultsrom[romIdx].name;
    m_strAdvice = g_faultsrom[romIdx].advice;

    if( g_faultsrom[romIdx].code==21 || g_faultsrom[romIdx].code==20 ) // ÁíÍâÒ»²àVCU/IDUÀëÏß
    {
    ...
    m_strDev = GetDeviceStr(g_faultsrom[romIdx].device,g_faultsrom[romIdx].deviceNo);
